...
export default function MusicBeat({
  ...
  return (
    ...
    >
      <AnimatePresence>
        {beat.description && tooltipVisible && (
          <motion.div
            initial={{ opacity: 0, y: 6 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 6 }}
            transition={{ duration: 0.18, ease: [0.25, 0.46, 0.45, 0.94] }}
            className="pointer-events-none absolute bottom-[calc(100%+8px)] left-0 w-48 bg-secondary text-surface/70 text-xs font-ibm rounded-lg px-2.5 py-1.5 shadow-md z-50 leading-snug"
          >
            {beat.description}
          </motion.div>
        )}
      ...
      <div className="flex items-center h-16 w-full rounded-xl overflow-hidden shadow-lg relative bg-on-surface border-secondary border-2">
        {/* Left info panel */}
        <div
          className={`flex items-center justify-between h-full px-3 lg:px-4 w-40 lg:w-52 rounded-md relative bg-background ${beat.description ? "cursor-pointer" : ""}`}
          onMouseEnter={() => beat.description && setTooltipVisible(true)}
